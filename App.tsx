...
const App: React.FC = () => {
  ...
  const output = useMemo(() => convertMarkdownToSocial(input), [input]);
  ...
  const handleInsertSyntax = useCallback((syntax: string) => {
    ...
    switch (syntax) {
      case "bold":
        newText = text.substring(0, start) + `**${selection || "bold"}**` + text.substring(end);
        newCursorPos = end + 4; 
        break;
      case "italic":
        newText = text.substring(0, start) + `*${selection || "italic"}*` + text.substring(end);
        newCursorPos = end + 2; 
        break;
      case "code":
        ...
        break;
      case "list":
        newText = text.substring(0, start) + `\n- ${selection || "item"}` + text.substring(end);
        newCursorPos = end + 3;
        break;
      default:
        return;
      ...
      <header className="bg-slate-900 text-white py-4 px-6 shadow-lg sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-xl tracking-tight">SocialMark</h1>
              <p className="text-xs text-slate-400 font-mono">Markdown to Unicode Converter</p>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
          ...
          <div className="flex flex-col h-full min-h-[500px]">
            <div className="mb-3 flex items-center justify-between">
              <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Markdown Input</label>
              {error && (
          ...
          <div className="flex flex-col gap-6">
             <div className="flex items-center justify-between mb-[-8px]">
              <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Preview</label>
              <button
                onClick={handleCopy}
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 text-xs text-blue-800">
              ...
              The text generated uses mathematical Unicode symbols. While they look great visually on Facebook, X (Twitter), and LinkedIn, screen readers may not parse them correctly. Use them for emphasis rather than essential information.
            </div>
          </div>
